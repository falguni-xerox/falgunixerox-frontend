...
export default function UploadPage() {
  ...
  const calculateLocalPrice = (pages, side, copyCount) => {
    ...
    let rate = 0;
    let billableUnits = pages;

    if (pages <= 5) {
      rate = 5;
    } else {
      rate = side === 'single'? 3 : 3.5;
    }

    if (side!== 'single') {
      billableUnits = Math.ceil(pages / 2);
    }

    return Math.round(billableUnits * rate * copyCount);
  };

  useEffect(() => {
    if (totalPages > 0) {
      const localPrice = calculateLocalPrice(billablePages, duplex, copies);
      setPrice(localPrice);
    }
  }, [jobId, copies, duplex, totalPages, printRange, customPages]);
  ...
  const handlePay = async () => {
    ...
    try {
      ...
      const orderRes = await axios.post(`${API_BASE_URL}/api/jobs/${jobId}/pay/checkout-order`, {
        copies,
        printType,
        printRange,
        customPages
      ...
      const options = {
        ...
        description: `${billablePages} Pages × ${copies} Copy - ${duplex}`,
        ...
        handler: async function (response) {
          try {
            ...
            setFinalToken('Online');
            setShowThankYou(true);
            setMessage('');
          } catch (err) {
            console.log(err);
            setMessage('Payment done but verify failed. Pay Cash at Counter.');
          }
        },
      ...
      console.log(error);
      setMessage('Payment order create failed');
    }
  };
  ...
  const handleCashPayment = async () => {
    ...
    try {
      ...
      console.log(error);
      setMessage('Cash order failed ❌');
    }
  };

  const totalSheets =
    duplex === 'single'
     ? billablePages * copies
      : Math.ceil(billablePages / 2) * copies;

  const getPriceBreakdown = () => {
    if (totalPages <= 0) return '';
    const rate = billablePages <= 5? 5 : (duplex === 'single'? 3 : 3.5);
    const billableUnits = duplex === 'single'? billablePages : Math.ceil(billablePages / 2);
    ...
    return (
      <div className="container" style={{ padding: '20px', maxWidth: '500px', margin: '0 auto', textAlign: 'center' }}>
        <div style={{ padding: '40px 20px', border: '2px solid #0a8f08', borderRadius: '12px', background: '#f0fff0' }}>
          ...
          <h2 style={{ color: '#0a8f08', margin: '20px 0' }}>Thank you for using Falguni Xerox!</h2>
          <p style={{ fontSize: '18px', margin: '10px 0' }}>Your Token Number:</p>
          <h1 style={{ fontSize: '48px', margin: '10px 0', color: '#ff9800' }}>#{finalToken}</h1>
          <p style={{ fontSize: '16px', color: '#666' }}>Printing Started...</p>
          <p style={{ fontSize: '14px', color: '#666', marginTop: '20px' }}>કાઉન્ટર પરથી તમારી પ્રિન્ટ લઈ લો</p>
          <button
            onClick={() => {
              setFile(null);
              setTotalPages(0);
        ...
        <div className="step" style={{ marginBottom: '20px', padding: '15px', border: '1px solid #ddd', borderRadius: '8px' }}>
          ...
          <div style={{ margin: '15px 0', padding: '10px', background: '#f9f9f9', borderRadius: '5px' }}>
            ...
            {printRange === 'custom' && customPages && (
              <p style={{ fontSize: '12px', color: '#666', marginLeft: '25px' }}>
              ...
              <input
                ...
                onChange={(e) => {
                  ...
                  if (val === '') {
                    ...
                    if (!isNaN(num) && num >= 1) setCopies(num);
                  }
                }}
                onBlur={() => {
      ...
      {price > 0 && jobId && (
        <div className="step" style={{ marginBottom: '20px', padding: '15px', border: '1px solid #ddd', borderRadius: '8px' }}>
          ...
          <p style={{ fontSize: '12px', color: '#666', marginTop: '10px', textAlign: 'center' }}>
            Online payment failed? Use Cash option
          </p>
        </div>
      )}
    </div>
